...
def Task2function(array):
    global connection, cursor
    # ask for N - the number of locations
    N = input("Enter number of locations:")
    
    # use sql to find the names, their coordinates and total populations
    df = pd.read_sql_query('''
    ...
    # use while loop to find the top N most populous neighbourhoods and add a circle to the map
    i = 0
    ...
    # order from least populous to most populous    
    df = df.sort_values(by = ['total_population'])
    
    # use while loop to find the top N least populous neighbourhoods and add a circle to the map
    i = 0
    while i < len(df) and (i < int(N) or df.iloc[i]["total_population"] == df.iloc[i-1]["total_population"]):
        folium.Circle(
            location = [df.iloc[i]['Latitude'], df.iloc[i]['Longitude']],
...
def Task3function(array):
    ...
    # instantialize the map
    m = folium.Map(location=[53.5444,-113.323], zoom_start=11)
    
    # ask for range, crime type, and number of neighbourhoods
    start = input('Enter start year (YYYY):')
    end = input('Enter end year (YYYY):')
    crime = input('Enter crime type:')
    num = input('Enter number of neighbourhoods:')
    
    # use sql to find the names, total incidents count, and coordinates
    cursor.execute('''
    ...
    # for each neighbourhood create a circle and add to the map
    for row in rows:
        folium.Circle(location=[float(row[2]), float(row[3])], 
                  popup= '%s <br> %d' % (row[0], int(row[1])),
                  radius= int(row[1])*5.0,
                  color= 'crimson',
                  fill= True,
                  fill_color= 'crimson'
                  ).add_to(m)
...
def Task4function(array):
    ...
    cursor.execute('''SELECT Column1.Neighbourhood_Name, Column2.pop, Column1.inc_count, (Column1.inc_count/ CAST(Column2.pop AS FLOAT)) AS Ratio, Column3.Latitude, Column3.Longitude
                      ...
                      WHERE ci.YEAR >= :sy AND ci.YEAR <= :ey
                      ...
                      WHERE Column1.Neighbourhood_Name = Column2.Neighbourhood_Name AND Column1.Neighbourhood_Name = Column3.Neighbourhood_Name and Column2.pop > 0
                      ORDER BY Ratio desc;''', {"sy":start_year, "ey":end_year})
    rows = cursor.fetchall()
    i = 0
    while i < len(rows) and (i < N or rows[i][3] == rows[i-1][3]):      # rows[i][3] is the ratio
    # This while loop will add a circle to the map for top N in neighbourhood
        ...
        cursor.execute('''SELECT col1.Crime_type, MAX(col1.ic)
                        ...
                        AND YEAR <= :ey
                        GROUP BY Crime_type
                        ORDER BY ic DESC ) AS col1''', {"nn":rows[i][0], "sy":start_year, "ey":end_year})   # rows[i][0] is the name
        crime_type = cursor.fetchone()
        #Adds the circle to the map
        folium.Circle(
            location = [rows[i][4], rows[i][5]], # rows[i][4] and rows[i][5] are the coordinates
            popup = rows[i][0] + '\n' + crime_type[0] + '\n' + str(rows[i][3]), # rows[i][0] is the name and rows[i][3] is the ratio
            radius = rows[i][3] * 2500.0,
            color = 'crimson',
            fill = True,
            fill_color = 'crimson').add_to(m)
        i+=1
